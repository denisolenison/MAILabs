...
template <class T> SListItem<T>::SListItem(T* figure) {
...
template <class T> SListItem<T>::SListItem(std::shared_ptr<T> figure) {
    this->figure = figure;
    this->next = nullptr;
	this->prev = nullptr;
    //std::cout << "List item: created" << std::endl;
...
template <class T> std::shared_ptr<T> SListItem<T>::GetValue() const {
    return this->figure;
}
